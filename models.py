...
class GPT(ClassificationBase):
    ...
    """
    backbone = GPT2ForTokenClassification
    ...
    def __init__(
        ...
        *args, **kwargs,
        ...
        super().__init__(*args, **kwargs)
        self.input_size = input_size
        self.backbone = self.backbone(config)
        self.config = config
...
GPT.forward.__doc__ = GPT.backbone.forward.__doc__
...
class Transformer(LightningBase):
    ...
    def __init__(
        ...
        *args, **kwargs,
        ...
        self.input_size = input_size
        self.backbone = self.backbone(config=enc_dec_config)
        self.config = config
        self.projection = nn.Sequential(
            nn.Linear(self.input_size, self.config.hidden_size),
            nn.LayerNorm(self.config.hidden_size),
...
Transformer.forward.__doc__ = Transformer.backbone.forward.__doc__